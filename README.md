...
当前省级轮廓来自公开行政区划 GeoJSON（DataV），经 **Douglas–Peucker 简化** 后投影为 SVG：

- 省界走向、邻接关系比「方块示意」准确得多  
- 仍是 **适度简化**（不是测绘级边界）；小岛礁会合并/省略  
- 九段线海域未完整绘制（避免把主图压扁）  
- 重建：`python3 scripts/build-map.py`（会下载/缓存 `100000_full.json`）
